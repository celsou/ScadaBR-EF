...
public class ShareUser implements JsonSerializable {
	...
	public void jsonDeserialize(JsonReader reader, JsonObject json) throws JsonException {
		...
		User user = Common.ctx.getUserCache().getUserDao().getUser(text);
		if (user == null)
		...
		if (StringUtils.isEmpty(text))
			throw new LocalizableJsonException("emport.error.missing", "accessType",
					ACCESS_CODES.getCodeList(ACCESS_OWNER));
		accessType = ACCESS_CODES.getId(text, ACCESS_OWNER);
		if (accessType == -1)
			throw new LocalizableJsonException("emport.error.invalid", "permission", text,
					ACCESS_CODES.getCodeList(ACCESS_OWNER));
	}
