...
public class DataPurge {
	...
	public static void schedule() {
		try {
			Common.timer.schedule((TimerTask) new DataPurgeTask());
	...
	private long purgePoint(DataPointVO dataPoint) {
		if (dataPoint.getLoggingType() == DataPointVO.LoggingTypes.NONE)
			// If there is no logging, then there should be no data, unless
			// logging was just changed to none. In either
			// case, it's ok to delete everything.
			return rm.purgeDataPointValues(dataPoint.getId());

		// No matter when this purge actually runs, we want it to act like it's
		// midnight.
		...
		if (files != null) {
			for (String filename : files) {
				...
				if (Collections.binarySearch(validIds, pointId) < 0) {
					// Not found, so the point was deleted from the database.
					// Delete the file.
